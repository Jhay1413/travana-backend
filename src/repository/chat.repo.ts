...
import { and, eq, ne, inArray, sql, desc, ilike, notExists } from "drizzle-orm"
...
export const chatRepo: ChatRepo = {
  ...
  markAllMessageAsRead: async (messageIds, userId) => {
    ...
    for (const messageId of messageIds) {
      await db
        ...
        .onConflictDoUpdate({
          target: [chatMessageRead.messageId, chatMessageRead.user_id_v2], // unique constraint or target columns
          set: {
            readAt: new Date(), // what to update if conflict
          },
        });
    }
  },
  ...
  getRooms: async (userId, filters, limit, cursor) => {
    ...
    const roomsWithUnreadCounts = await Promise.all(
      userRooms.map(async (room) => {
        ...
        const unreadCount = Number(unread.count);
        ...
        return {
          ...
          participants: room.participants.map(p => ({
            id: p.participantId!,
            name: `${p.user_v2?.firstName ?? ''} ${p.user_v2?.lastName ?? ''}`.trim(),
          ...
          lastMessage: room.messages[0]
            ? {
              ...
              senderName: `${room.messages[0].sender_v2?.firstName ?? ''} ${room.messages[0].sender_v2?.lastName ?? ''}`.trim(),
              ...
              attachments:
                room.messages[0].attachments?.length
                  ? room.messages[0].attachments
                  : [],
            }
            : undefined,
          name:
  ...
  getMessages: async (filters, roomId) => {
    ...
    // ✅ 5. Compute per-user "isRead" for each message
    const formattedMessages = messagesToReturn.map((msg) => {
      const hasRead = msg.reads.some(
        (r) => r.user_id_v2 !== msg.sender_id
      );
      ...
      return {
        ...
        senderName: `${msg.sender_v2?.firstName ?? ''} ${msg.sender_v2?.lastName ?? ''}`.trim(),
        timestamp: msg.timestamp?.toISOString() || 'No date',
        isRead: hasRead, // ✅ true if this user has a read record
  ...
  countUnreadMessages: async (userId: string) => {
    // Find all active rooms the user is in
    const userRooms = await db.query.chatRoom.findMany({
      where: (room, { eq }) => eq(room.isActive, true),
      with: {
        participants: {
    ...
    for (const room of rooms) {
      ...
      totalUnreadCount += Number(result.count);
    }
