        ...
        for (const ticketItem of unSnoozedTickets) {
          ...
          if (recipientId && ticketItem.due_date) {
            ...
            const notificationData = {
              ...
              message: isOverdue
                ? hoursOverdue > 0
                  ? `Ticket "${ticketItem.subject || 'Untitled'}" is ${hoursOverdue} hour(s) overdue!`
                  : `Ticket "${ticketItem.subject || 'Untitled'}" is overdue!`
                : `Ticket "${ticketItem.subject || 'Untitled'}" is due soon!`,
              reference_id: ticketItem.id,
              due_date: ticketItem.due_date.toISOString(),
              client_id: ticketItem.client_id,
            };
